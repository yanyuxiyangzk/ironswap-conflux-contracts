...
contract MigratorFactory is Pauseable {
    ...
    mapping(address => address) public cIronLpToken;
    ...
    function setIronLpToken(address token0, address token1, address _ironLpToken) external {
        require(msg.sender == operatorAddr, "IronSwap: FORBIDDEN");
        address pair = getPair[token0][token1];
        require(pair != address(0), "IronSwap: pair no create");
        cIronLpToken[pair] = _ironLpToken;
    }
    ...
    function setDesiredLiquidity(address token0, address token1, uint _desiredLiquidity, uint _multiplier) external {
        ...
        address pair = getPair[token0][token1];
        require(pair != address(0), "IronSwap: pair no create");
        require(_multiplier > 0, "IronSwap: multiplier must setting");
