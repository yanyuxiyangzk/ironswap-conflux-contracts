...
interface IMigratorFactory {
    ...
    function cIronLpToken(address pair) external view returns (address);
...
contract MigratorPair is IERC777Recipient {
  ...
  modifier whenPaused() {
     require(!IMigratorFactory(factory).getPause(), "Pauseable: IronSwap paused");
      _;
  }
  ...
  // user cIronLpToken Exchange
  function exchangeLp() external {
      address cIronLpToken = IMigratorFactory(factory).cIronLpToken(address(this));
      require(cIronLpToken != address(0), "IronSwap: cIronLpToken is ZERO_ADDRESS");
      uint amount = IERC20(cIronLpToken).balanceOf(msg.sender);
      require(amount > 0, "IronSwap: no balance");
      IERC20(cIronLpToken).safeTransferFrom(msg.sender, address(this), amount);
  ...
  function _exchangeLp(address from, uint amount) internal {
      ...
      require(swapPair != address(0), "IronSwap: no swap pair");
      uint _multiplier = IMigratorFactory(factory).getInflationPair(address(this));
      require(_multiplier > 0, "IronSwap: multiplier is zero");
  ...
  function addLiquidity() external {
      ...
      require(msg.sender == operatorAddr, "IronSwap: FORBIDDEN");
      address swapFactory = IMigratorFactory(factory).swapFactory();
      // when finish addLiquidity close the method
      require(swapFactory != address(0), "IronSwap: no swap factory");
      ...
      require(balance0 > 0 && balance1 > 0, "IronSwap: balance is zero!");
  ...
  function setTotalShareAmount(uint _shareAmount) external {
    require(_shareAmount > 0, "IronSwap: shareAmount is zero");
    address operatorAddr = IMigratorFactory(factory).operatorAddr();
    require(msg.sender == operatorAddr, "IronSwap: FORBIDDEN");
  ...
  function tokensReceived(address operator, address from, address to, uint amount,
        ...
        address cIronLpToken = IMigratorFactory(factory).cIronLpToken(address(this));
        if(cIronLpToken == msg.sender) {
           // Exchange IronSwap Liquidity
