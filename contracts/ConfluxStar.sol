...
contract ConfluxStar is Ownable, Pauseable, IERC777Recipient {
  ...
  // Iron crosschain atom mapping
  address public cIronToken;
  ...
  constructor(
    ...
    function ironBalance() external view returns(uint256) {
      return IERC20(cIronToken).balanceOf(address(this));
    }

    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBal = IERC20(cIronToken).balanceOf(address(this));
        require(_amount <= tokenBal, "ConfluxStar: Balance insufficient");
        IERC20(cIronToken).transfer(_to, _amount);
    }
    ...
    function accountList(uint256 begin) public view returns (address[100] memory) {
        require(begin >= 0 && begin < _accountList.length, "IronSwap: accountList out of range");
