...
library UniswapV2Library {
    ...
    function wrapperAddress(address a) private pure returns (address addr) {
        bytes memory b = toBytes(a);
        b[0] = b[0] & 0x0F;
        b[0] = b[0] | 0x80;
    ...
    function subBytes(bytes memory data, uint start, uint length) internal view returns(bytes memory){
      bytes memory _a3 = new bytes(length - start);
